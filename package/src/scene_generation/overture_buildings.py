...
def load_overture_buildings_for_aoi(
    ...
    *,
    ...
    if df.empty:
        return gpd.GeoDataFrame(df.drop(columns=["geometry_wkb"], errors="ignore"), geometry=[], crs="EPSG:4326")
...
def _load_duckdb_extensions(con) -> None:
    ...
    for extension in ("httpfs", "spatial"):
        try:
            ...
            logger.debug("DuckDB INSTALL %s failed or was unnecessary: %s", extension, exc)
